...
function TextSubmitPage() {
  const defaultText = "PREFIX ex:<http://example.org/station/> \nPREFIX sosa:<http://www.w3.org/ns/sosa/> \nPREFIX xsd: <http://www.w3.org/2001/XMLSchema#>\nSELECT ?observedProperty ?result ?resultTime\nFROM<http://example.org/dataset>\nWHERE{ ?observation a sosa:Observation ; sosa:hasFeatureOfInterest ex:omm_station_07005 ; sosa:observedProperty ?observedProperty ; sosa:hasSimpleResult ?result ; sosa:resultTime ?resultTime FILTER ( ( ?observedProperty = ex:Temperature ) && ( ?resultTime >= \"2024-10-25T00:00:00\"^^xsd:dateTime ) && ( ?resultTime < \"2024-10-26T00:00:00\"^^xsd:dateTime ) ) } ORDER BY ?resultTime";
